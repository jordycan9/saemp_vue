  ...
  <v-flex>
        ...
        <v-toolbar flat color="white">
         ...
         <v-btn v-if="verNuevo==0" @click="mostrarNuevo()"  small>Nueva Solicitud</v-btn>
          ...
          <v-dialog v-model="adModal" max-width="290">
                   ...
                   <div id="solicitudGuardia"> <br><br>
                          ...
                          <div id="datos">
                              <p id="encabezado">
                                  ...
                                  <strong>Solicitud Unica de Arribo</strong><br>
                          ...
                          <div id="fact">
                            ...
                            Frontera, Tab. a: {{fecha_real }}<br>
                             Folio: <strong>SU{{agencia.folio}}{{folioId}}</strong><br>
            ...
            <p class="text-monospace">Como representante de la embarcación abajo mencionada, cuyas caracteristicas detallo, solicito a usted 
              Autorización para el atraque y uso de infraestructura del recinto portuario
            </p>
                        ...
                        <div class="col-md-8">
                          <table class="table table-sm">
                            <thead class="thead-dark">
                              <tr>
                                <th colspan="5">Datos del Buque y estadía</th>
                            ...
                            <tbody>
                              <tr>
                                ...
                                <td><strong>{{fechaSolicitada}}</strong></td>  
                              
                              </tr>
                              <tr>
                                <td>Nombre del Buque</td>
                                <td><strong>{{embarcacion.nombre}}</strong></td>
                              </tr>
                              <tr> 
                                <td>Armador</td>
                                <td><strong>{{embarcacion.armador}}</strong></td>
                              </tr>
                              <tr> 
                                <td>Procedencia</td>
                                <td><strong>{{procedencia_solicitud}}</strong></td>
                              </tr>
                              <tr> 
                                <td>ETA</td>
                                <td><strong>{{etaReporte}}</strong></td>
                              </tr>
                                <tr> 
                                <td>ETD</td>
                                <td><strong>{{etdReporte}}</strong></td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                      ...
                      <div class="col-md-10">
                        <table class="table table-sm">
                            ...
                            <tbody>
                              <tr>
                                <td>Eslora</td>
                                <td><strong>{{embarcacion.eslora}}</strong></td>
                                <td>Manga</td>
                                <td><strong>{{embarcacion.manga}}</strong></td>
                                 <td>Calado</td>
                                <td><strong>{{embarcacion.calado}}</strong></td>
                              </tr>
                              <tr>
                                <td>TBR</td>
                                <td><strong>{{embarcacion.tbr}}</strong></td>
                                <td>TRN</td>
                                <td><strong>{{embarcacion.trn}}</strong></td>
                                <td>Tipo</td>
                          ...
                          Aprobacion AMLS: {{aprobacionAmls}}<br>
                          Aprobacion APITAB: {{aprobacionApi}}
                      </div>
                     </div>
    ...
    <v-container grid-list-sm class="pa-4 white" v-if="verNuevo">
        <v-layout row wrap>       
            ...
            <v-flex xs12 sm4 lg4 x14>
               <v-autocomplete  :items="procedencias" v-model="procedencia" label="Procedencia"></v-autocomplete> 
            </v-flex>
           ...
           <v-flex xs12 sm4 lg4 x14>
            ...
            <v-flex xs12 sm4 lg4 x14>
             ...
             <v-flex xs12 sm4 lg4 x14>
              <v-autocomplete :items="muelles"  v-model="muelle" label="Muelle"></v-autocomplete>
            </v-flex>
      ...
      return{
        ...
        methods: {
          ...
          mostrarComprobante(item){
            ...
            var fecha3 = new Date(item.createdAt);
            this.fechaSolicitada = fecha3.toUTCString()
            this.fecha_real = fecha2.toLocaleDateString("es-ES", options);
            this.agencia = item.agencia
            this.folioId = item.folio_solicitud
            this.etaReporte = item.eta
            this.etdReporte = item.etd
          ...
          validar(){
            ...
            if(!this.agencia){
              this.validaMensaje.push('El nombre de la agencia es obligatoria');
            }
            if(!this.comentarios){
