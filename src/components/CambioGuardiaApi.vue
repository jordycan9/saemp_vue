            ...
            <v-card>
              ...
              <v-card-actions>
                ...
                <v-btn @click="activarDesactivarCerrar()" color="error" small>
                 ...
                 <v-btn v-if="adAccion==1" @click="activarApi()" small color="primary">
                  Aprobar
                </v-btn>
                <v-btn v-if="adAccion==2" @click="activar()" small color="orange" >
                  Anular
                </v-btn>
  ...
  export default {
    ...
    data(){
      return{
          ...
          computed: {
          formTitle () {
            return this.editedIndex === -1 ? 'Nueva Cambio de Guardia' : 'Editar Cambio de Guardia'
          }
        },
        ...
        activar(){
          let me=this;
           let header={"Token":this.$store.state.token};
           let configuracion= {headers: header}
           axios.put('cambioGuardia/deactivateApi',{'_id':this.adId,'aprobacionApi':this.$store.state.usuario.nombre},configuracion)
               .then(function(response){
                 me.adModal=0,
                 me.adAccion=0,
                 me.adNombre='',
        ...
        desactivar(){
         ...
         activarApi(){
          let me=this;
           let header={"Token":this.$store.state.token};
           let configuracion= {headers: header}
           axios.put('cambioGuardia/aprobarApi',{'_id':this.adId,'aprobacionApi':this.$store.state.usuario.nombre},configuracion)
               .then(function(response){
                 me.adModal=0,
                 me.adAccion=0,
                 me.adNombre='',
